// TLE due to too many set operations.
...
void add(int i) {
    if(pt[i]) 
        cur.erase({a[i][pt[i]], i});
    sig.erase({a[i][pt[i] + 1], i});
    if(!pt[i])
        dub.erase({a[i][1] + a[i][2], i});
    pt[i]++;
    ans += a[i][pt[i]];
    cur.emplace(a[i][pt[i]], i);
    sig.emplace(a[i][pt[i] + 1], i);
}
